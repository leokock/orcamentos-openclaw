# Benchmark — SC-Litoral-Norte | medio-alto

**Tipo de combinação:** Região × Padrão
**N projetos:** 9
**Gerado:** 2026-04-19T10:21:01

---

## Perfil financeiro

### R$/m² (projetos válidos, AC ≥ 1.000 m², R$/m² ∈ [500, 10k])

| Percentil | R$/m² |
|---|---:|
| mín | R$ 1,918 |
| p25 | R$ 2,663 |
| **mediana** | R$ 2,878 |
| p75 | R$ 3,516 |
| máx | R$ 7,160 |
| CV (coef. variação) | 0.53 |
| N | 6 |

### Área construída

| Percentil | AC (m²) |
|---|---:|
| mín | 1,403 |
| mediana | 6,702 |
| máx | 15,603 |

### Total orçamento

- **Mediana total:** R$ 30,584,965
- Faixa: R$ 3,735,761 — R$ 50,516,883

**UR mediana:** 67

## Perfil estrutural

| Indicador | Mediana | n |
|---|---:|---:|
| Concreto (m³/m² AC) | 0.342 | 5 |
| Taxa aço (kg/m³ concreto) | 75.2 | 4 |
| Forma (m²/m² AC) | 1.809 | 5 |

## Distribuição % Macrogrupo (mediana)

| Macrogrupo | % do total |
|---|---:|
| Gerenciamento | 33.8% |
| Supraestrutura | 22.0% |
| Outros | 10.3% |
| Instal Geral | 9.2% |
| Esquadrias | 7.8% |
| Hidrossanitaria | 5.7% |
| Infraestrutura | 5.5% |
| Sist Especiais | 5.2% |
| Complementares | 4.8% |
| Pisos | 4.7% |
| Pint Interna | 4.6% |
| Pintura Geral | 4.4% |
| Rev Parede | 3.5% |
| Fachada | 3.2% |
| Eletrica | 2.3% |
| Alvenaria | 2.2% |
| Rev Teto | 1.8% |
| Impermeabilizacao | 1.4% |
| Loucas | 0.9% |
| Climatizacao | 0.7% |

## Top clientes nesta combinação

- **paludo**: 2 projeto(s)
- **all**: 1 projeto(s)
- **amalfi**: 1 projeto(s)
- **by**: 1 projeto(s)
- **grandezza**: 1 projeto(s)

## Projetos (9)

| Projeto | AC (m²) | R$/m² | Tipologia | Cidade |
|---|---:|---:|---|---|
| `all-lago-di-garda` | 7,056 | R$ 7,160 | rv_alto | Porto Belo |
| `amalfi-tramonti` | 15,603 | N/D | rv_medio_alto | Bombinhas |
| `by-seasons-by-seasons` | 6,349 | N/D | rv_medio_alto | Itapema |
| `grandezza-gran-royal` | 5,225 | R$ 1,918 | rv_medio_alto | Balneário Piçarras |
| `holze-sense-106` | 8,700 | R$ 3,516 | rv_medio_alto | Porto Belo |
| `mabrem-gran-torino` | 12,519 | R$ 2,974 | rv_alto | Piçarras |
| `paludo-barbados` | 1,728 | R$ 2,781 | rv_medio_alto | Bombinhas |
| `paludo-nassau` | 1,403 | R$ 2,663 | rv_alto | Bombinhas |
| `thozen-mirador-de-alicante` | 475 | R$ 73,778 | rv_alto | Porto Belo |

## Interpretação automática

- Em `SC-Litoral-Norte`, empreendimentos medio-alto custam tipicamente **R$ 2,878/m²** (faixa p25-p75: R$ 2,663–R$ 3,516).
- CV 0.53 — variação **alta**. Projetos nesta combinação são heterogêneos; usar com cuidado.
