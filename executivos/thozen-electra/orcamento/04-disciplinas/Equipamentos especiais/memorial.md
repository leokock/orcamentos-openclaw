...
<!-- STATUS: aguardando-cross-disciplinas -->

> ⚠ **ATENÇÃO — DISCIPLINA CONSOLIDADA**
>
> Equipamentos Especiais é uma **aba-agregadora**: ela consolida equipamentos eletromecânicos
> que dependem de decisões e quantitativos tomados em **outras disciplinas**.
>
> **Esta aba deve ser a ÚLTIMA a ser fechada**, só depois que:
> - Elétrico, Hidrossanitário, Sprinklers, PPCI, Climatização, Automação, Telecomunicação
>   e Iluminação estiverem processadas
> - Os equipamentos "definitivos" (cotados, dimensionados) forem extraídos dos projetos
>   de cada disciplina e confirmados
>
> Ver seção **"Varredura cross-disciplinas"** no final deste memorial para o checklist
> que o Leo vai rodar ao final do processamento.
...
## 🔍 Varredura cross-disciplinas (a fazer AO FIM do orçamento)

Esta seção é o **checklist que Leo roda por último**, depois que as outras disciplinas
estiverem processadas. Pra cada linha, validar se o equipamento está **de fato aqui** ou se
foi absorvido pela disciplina-origem (evitando double-count).

### Matriz de origem dos equipamentos

| Grupo de Equip. Especiais | Disciplina-fonte primária | O que confirmar na disciplina-fonte | Decisão final |
|---|---|---|---|
| **Grupo 1 — Elevadores** | Projeto arquitetônico | Confirmar 4 sociais c/CM + 2 sem CM + 2 serviço. Cotação real de fornecedor (Otis/ThyssenKrupp/Atlas) | Fica em Equip. Esp. (eletromecânico) |
| **Grupo 2 — Grupo Gerador** | [Elétrico](../Elétrico/) | Dimensionamento oficial do gerador (kVA) baseado em cargas essenciais calculadas | Fica em Equip. Esp. (mecânico) |
| **Grupo 3 — Bombas de Recalque/Pressurização** | [Hidrossanitário](../Hidrossanitário/) | Vazão total, altura manométrica, número de conjuntos. Confirmar se bombas estão lá ou aqui | **CONFIRMAR onde orçar** (pode estar em Hidro) |
| **Grupo 4 — PCI Hidromecânico** | [Sprinklers](../Sprinklers/) + [PPCI](../PPCI/) | RTI (60 m³), bomba principal 25 CV, jockey. Evitar double-count com Sprinklers (que pode ter bombas também) | **CONFIRMAR**: PCI hidromecânico aqui ou em Sprinklers? |
| **Grupo 5 — SPDA** | Depende — podendo estar na disciplina "SPDA" se existir, senão em [Elétrico](../Elétrico/) | Conferir DWG 11 SPDA: captores, cabos 50/35/25mm², hastes, conexões | ⚠ Electra NÃO tem pasta SPDA — provavelmente **fica aqui** |
| **Grupo 6 — Exaustão Churrasqueiras** | [Climatização](../Climatização/) ou disciplina própria | Qtd de churrasqueiras (projeto lazer), dimensionamento de exaustor/coifa | **CONFIRMAR** escopo — há pasta "13 CHURRASQUEIRA EXAUSTAO" nos projetos |
| **Grupo 7 — Ar-Condicionado (VRF)** | [Climatização](../Climatização/) | VRF é climatização. Esta aba deveria ter SÓ os equipamentos grandes (condensadoras, evaporadoras), e Climatização cuida de tubulação + mão de obra | **CONFIRMAR divisão**: equipamentos aqui, instalação lá? |
| **Grupo 8 — Ventilação Mecânica** | Disciplina de ventilação (há pasta "12 ESCADA VENTILACAO MECANICA" em projetos) | Ventiladores, dampers, dutos. Electra tem projeto legal detalhado (DXF scanned) | **CONFIRMAR** se orça aqui ou cria disciplina própria |
| **Grupo 9 — Interfone/Vídeo Porteiro** | [Telecomunicação](../Telecomunicação/) | Monitores, centrais, câmeras | **CONFIRMAR**: pertence a Telecom ou Equip. Esp.? |
| **Grupo 10 — CFTV** | [Telecomunicação](../Telecomunicação/) | Câmeras, NVR, cabeamento CAT6 | **CONFIRMAR** com Telecom |
| **Grupo 11 — Iluminação Emergência** | [PPCI](../PPCI/) ou [Iluminação](../Iluminação/) | Luminárias emergência, blocos autônomos, central | **CONFIRMAR**: é PPCI (sistema de emergência) ou Iluminação (contagem simples) |
| **Grupo 12 — Automação BMS** | [Automação](../Automação/) | Controladora central, módulos I/O, SCADA, integrações | **CONFIRMAR divisão**: hardware aqui, integrações/parametrização lá? |

### Checklist a rodar ao fim (depois de processar todas as outras)

- [ ] **Elevadores (Grupo 1)** — cotação real recebida? Bater com qtd em [lazer.md](../../00-projeto/lazer.md) (6 elevadores: 4 sociais + 2 emergência)
- [ ] **Grupo Gerador (Grupo 2)** — dimensionamento kVA confere com carga essencial calculada em [Elétrico](../Elétrico/)?
- [ ] **Bombas (Grupo 3)** — decidir se orçamento fica aqui ou em [Hidrossanitário](../Hidrossanitário/). **Não pode estar nos dois**.
- [ ] **PCI Hidromecânico (Grupo 4)** — decidir se RTI+bombas ficam aqui ou em [Sprinklers](../Sprinklers/)/[PPCI](../PPCI/). Não pode duplicar.
- [ ] **SPDA (Grupo 5)** — cabeamento dimensionado conforme NBR 5419 na altura real da Electra (~77,76 m torre)
- [ ] **Exaustão (Grupo 6)** — qtd de churrasqueiras confirmada com memorial descritivo/projeto lazer
- [ ] **Ar-condicionado (Grupo 7)** — DWG HVAC processado (`RA_ARC_EXE_00_TODAS CAD_R05.dwg`)? Equipamentos detalhados (não mais `cj=1`)
- [ ] **Ventilação (Grupo 8)** — qtd de dampers/dutos bate com projeto legal (DXF scanned hoje)
- [ ] **Interfone (Grupo 9)** — 96 monitores confirmado (24 aptos × 2 torres × 2 monitores/apto)? Ou é 192 (1/apto)?
- [ ] **CFTV (Grupo 10)** — layout de câmeras batido com planta arquitetônica de garagens e lazer
- [ ] **Iluminação Emergência (Grupo 11)** — Critério NBR 10898 aplicado com áreas reais (1 a cada ~10m corredor)
- [ ] **BMS (Grupo 12)** — lista de subsistemas integrados finalizada (após outros grupos fechados)
- [ ] **Divergência elevadores** (original do memorial): EPCs!D38 = 3 vs Equip. Esp. = 6. Resolver antes de fechar
- [ ] **Divergência monitores** (96 vs 192): confirmar número real

### Após checklist concluído

1. Gerar `quantitativos.xlsx` desta disciplina (script: `gerar_quantitativos_equipamentos_especiais.py`)
2. Marcar pendência do painel como resolvida
3. Rodar `atualizar_painel.py`
4. Esta disciplina fica **✅ Processada**, fechando o orçamento executivo
