...
# -*- coding: utf-8 -*-
"""
Viabilidade Financeira — Placon Armínio Tavares (Florianópolis / Centro)
Adaptado de gerar_viabilidade_arthen_arboris.py.

Consome: parametrico-placon-hermann-blumenau.xlsx + premissas-viabilidade.json
Produz : viabilidade-placon-hermann-blumenau-v1.xlsx (analítico 7 abas)
         viabilidade-placon-hermann-blumenau-v1-didatico.xlsx (didático 9 abas)

Modelo: VPL, TIR, Monte Carlo 10k iter ancorado no paramétrico V2 Híbrido,
        tornado, cenários, fluxo mensal didático (3 pré + 24 obra + 12 pós).

Base custo: R$ 17.865.737,68 (Grand Total V2 Híbrido pós-override Gerenciamento).
Prazo obra: 24 meses. 55 UR, AC 4.077,29 m², privativa 2.083,94 m².
"""
...
# PATHS
# ============================================================================
PACOTE_DIR = Path(r"C:\Users\leona\orcamentos-openclaw\base\pacotes\placon-hermann-blumenau")
DRIVE_DIR = Path(
    r"G:\Drives compartilhados\03 CTN Projetos\2. Projetos em Andamento"
    r"\_Parametrico_IA\hermann-blumenau"
)
# Fonte do custo: paramétrico V2 Híbrido local (git), não no Drive (arquivo
# Drive tem nome diferente — parametrico-v3-hibrido). Lemos do git onde é fresco.
PARAMETRICO_XLSX = PACOTE_DIR / "parametrico-placon-hermann-blumenau.xlsx"
PREMISSAS_JSON = PACOTE_DIR / "premissas-viabilidade.json"
OUTPUT_XLSX = PACOTE_DIR / "viabilidade-placon-hermann-blumenau-v1.xlsx"
OUTPUT_XLSX_V2 = PACOTE_DIR / "viabilidade-placon-hermann-blumenau-v1-didatico.xlsx"
PLOTS_DIR = PACOTE_DIR / "_viabilidade_plots"

# Fallback do custo paramétrico quando a planilha não tem cache e a avaliação
# via `formulas` falha: Grand Total pós-override Gerenciamento V2 Híbrido.
CUSTO_OBRA_FALLBACK = 24_445_535.95
...
def load_parametrico():
    ...
    # Fallback: xlsx sem cache de valores → tentar avaliar fórmulas via
    # `formulas` lib; se também falhar, usar CUSTO_OBRA_FALLBACK com breakdown
    # proporcional ao arthen (placeholder pra visualização — dist real só pós-abrir
    # no Excel).
    print(f"      [parametrico sem cache — tentando avaliar via formulas lib]")
    total = 0.0
    breakdown = {}
    try:
        ...
        print(f"      [formulas lib falhou: {exc} — usando fallback hardcoded]")

    if total <= 0:
        # Fallback duro: usa custo total hardcoded + breakdown proporcional padrão
        # V2 Híbrido (pesos típicos pós-override Gerenciamento placon).
        print(f"      [usando CUSTO_OBRA_FALLBACK = R$ {CUSTO_OBRA_FALLBACK:,.2f}]")
        total = float(CUSTO_OBRA_FALLBACK)
        # Pesos aproximados de um projeto Médio-Alto 24m (referência arthen ajustada).
        # Serve apenas pra Gantt/stacked — fluxo de caixa usa curva_mensal_obra_pct.
        pesos_default = {
            "Gerenciamento": 0.055,
            "Mov. Terra": 0.015,
            "Infraestrutura": 0.085,
            "Supraestrutura": 0.165,
            "Alvenaria": 0.065,
            "Impermeabilização": 0.020,
            "Instalações": 0.120,
            "Sist. Especiais": 0.035,
            "Climatização": 0.030,
            "Rev. Int. Parede": 0.055,
            "Teto": 0.025,
            "Pisos": 0.060,
            "Pintura": 0.035,
            "Esquadrias": 0.075,
            "Louças e Metais": 0.035,
            "Fachada": 0.060,
            "Complementares": 0.045,
            "Imprevistos": 0.020,
        }
        s = sum(pesos_default.values())
        for nome, w in pesos_default.items():
            pct = w / s
            breakdown[nome] = {"valor": total * pct, "pct": pct}
...
def build_capa(ws, prem, kpis, mc_stats):
    ...
    rows = [
        ...
        ("VGV líquido (pós-permuta)", kpis["vgv_liquido"], FMT_BRL),
    ...
    r += 2
    ws.merge_cells(f"A{r}:F{r}")
    c = ws.cell(row=r, column=1, value="Fonte custo: parametrico-placon-hermann-blumenau.xlsx CUSTOS_MACROGRUPO!D22")
    c.font = FONT_SMALL
    c.alignment = ALIGN_LEFT
    r += 1
    ws.merge_cells(f"A{r}:F{r}")
    c = ws.cell(row=r, column=1, value=f"Fonte preço venda: referência Centro Florianópolis Médio-Alto abr/2026 — R$ {prem['receita']['vgv_m2_base']:,.0f}/m² privativo")
    c.font = FONT_SMALL
    c.alignment = ALIGN_LEFT
...
def build_premissas(ws, prem, kpis):
    ...
    c = ws["A1"]
    c.value = "PREMISSAS — editar valores e regerar via gerar_viabilidade_placon_arminio.py"
    c.fill = FILL_DARK
    c.font = FONT_TITLE
    c.alignment = ALIGN_CENTER
    ...
    r = row_input(r, "VGV R$/m² privativo", rec["vgv_m2_base"], FMT_BRL2, "Centro Florianópolis médio pesquisa 2026")
    r = row_input(r, "VGV bruto (R$)", kpis["vgv_bruto"], FMT_BRL, "área_priv × R$/m²")
    r = row_input(r, "VGV líquido permuta (R$)", kpis["vgv_liquido"], FMT_BRL, f"após {int(proj['permuta_pct_unidades']*100)}% de permuta")
...
def build_fluxo_caixa(ws, fluxo, prem, fluxo_png=None):
    ...
    _com = prem.get("comercial", {})
    _cor_pct = int(round(_com.get("corretagem_pct", 0.05) * 100))
    _mkt_pct = int(round(_com.get("marketing_pct", 0.03) * 100))
    _ret_pct = int(round(_com.get("ret_pct", 0.04) * 100))
    _adm_pct = int(round(_com.get("adm_incorporadora_pct", 0.02) * 100))
    write_line(r, f"  Corretagem ({_cor_pct}% VGV — no mês da venda)", fluxo["corretagem"]); r += 1
    write_line(r, f"  Marketing ({_mkt_pct}% VGV — linear na obra)", fluxo["marketing_mensal"]); r += 1
    write_line(r, f"  RET {_ret_pct}% (Patrimônio de Afetação)", fluxo["ret"]); r += 1
    write_line(r, f"  Admin incorporadora ({_adm_pct}% VGV — linear)", fluxo["adm_mensal"]); r += 1
...
def build_resultado(ws, prem, kpis, mc_stats, fluxo, r_fluxo_liquido_in_fc):
    ...
    rows = [
        ...
        ("VGV líquido permuta", kpis["vgv_liquido"], None, "pós permuta", ""),
...
# ============================================================================
# V2 — LAYOUT DIDÁTICO (para diretor Placon)
# ============================================================================
...
def build_parecer(ws, prem, kpis, mc_stats, cenarios, tornado):
    ...
    _write_kpi_box(
        ...
        "Quanto a Placon precisa ter em caixa (ou via SFH) no mês mais apertado, antes das vendas cobrirem os custos",
    ...
    permuta_pct = proj.get('permuta_pct_unidades', 0.0)
    permuta_frase = (
        f"O terreno será adquirido por permuta física de {int(permuta_pct*100)}% das unidades, eliminando desembolso monetário com o landowner.\n\n"
        if permuta_pct > 0
        else "O terreno é adquirido via compra direta (sem permuta física assumida no modelo). Ajustar se houver estrutura de permuta no contrato final.\n\n"
    )
    narr1 = (
        f"O {proj['nome']} é um empreendimento residencial + comercial da {proj['cliente'].split(' ')[0]} com {proj['n_unidades']} unidades "
        f"(tipologia Misto — studios + 1-2 dormitórios), distribuídas em "
        f"{proj['area_construida_m2']:,.0f} m² de área construída, localizado no bairro {proj['bairro']} em {proj['cidade']}/{proj['estado']} — "
        f"região central de alta densidade e demanda por produto compacto.\n\n"
        ...
        f"(referência Centro Florianópolis Médio-Alto, abr/2026). Estrutura padrão de mercado: "
    ...
    atencao = [
        f"⚠  Exposição máxima de caixa: R$ {kpis['exposicao_max']/1e6:.1f} milhões. A Placon precisa ter esse capital disponível (próprio ou via SFH) nos meses mais apertados do cronograma.",
    ...
    recomendacao = (
        f"Com base nos indicadores apurados, o Armínio Tavares é um projeto VIÁVEL e RECOMENDADO para avanço. "
        ...
        f"(1) Validar o preço de venda de R$ {prem['receita']['vgv_m2_base']:,.0f}/m² com corretores locais de Centro Florianópolis "
        f"e comparáveis recém-lançados — é o fator de maior impacto sobre o resultado;\n"
        f"(2) Confirmar capacidade financeira da Placon para suportar a exposição máxima de R$ {kpis['exposicao_max']/1e6:.1f}M "
        f"(próprio ou via financiamento à produção SFH);\n"
        f"(3) Revisar a velocidade de vendas assumida (50% vendido no mês {prem['curva_vendas']['t50']} do modelo) contra o histórico da Placon em "
        f"projetos similares — Centro Florianópolis é mercado aquecido, mas a curva real precisa calibrar contra dados da incorporadora."
    )
...
def build_como_ler(ws, prem=None, kpis=None, mc_stats=None, cenarios=None, tornado=None):
    ...
    glossario = [
        ("VPL — Valor Presente Líquido",
         ...
         f"→ Neste estudo: VPL = {_fmt_m(vpl_m)}. Indica que o Armínio Tavares produz {_fmt_m(vpl_m)} a MAIS do que o mínimo exigido "
        ...
        ("VGV — Valor Geral de Vendas",
         ...
         f"  • VGV BRUTO: valor total se todas as unidades fossem vendidas pela Placon\n"
         f"  • VGV LÍQUIDO: após subtrair a permuta física ({int(permuta_pct*100)}% das unidades vão para o proprietário do terreno)\n\n"
         f"→ Neste estudo: VGV bruto {_fmt_m(vgv_bruto)}; VGV líquido (que a Placon de fato vende) {_fmt_m(vgv_liq)}."),
        ...
        ("Margem líquida",
         ...
         f"→ Neste estudo: {_fmt_pct(margem)} — acima da média do segmento, puxada por preço forte em Centro Florianópolis e custo calibrado no paramétrico."),
        ...
        ("Permuta física",
         ...
         f"→ Neste estudo: {int(permuta_pct*100)}% das unidades vão para o landowner. VGV vendável pela Placon cai de "
        ...
        ("Paramétrico V2 Híbrido",
         ...
         f"Armínio Tavares em R$ {custo_total/ac:,.0f}/m² — coerente com padrão Médio-Alto."),
    ]
...
def build_detalhe_custo(ws, prem, breakdown, fluxo, gantt_png, curva_s_png, stacked_png):
    ...
    texto_metodologia = (
        ...
        f"1)  QUANTITATIVOS — extraídos do modelo BIM (IFC) do Armínio Tavares: volumes de concreto, área de fôrma, m² de alvenaria, "
        f"m² de revestimento, kg de aço, unidades de esquadrias, etc. Para o Armínio Tavares: {prem['projeto']['area_construida_m2']:,.0f} m² AC, "
        ...
        f"3)  ÍNDICES DERIVADOS — 29 índices que ajustam o custo final conforme o briefing do Armínio Tavares: fundação em hélice "
        ...
        f"R$ 3.337/m², faixa P25-P75 R$ 2.974-3.710. Armínio Tavares em R$ {custo_total/ac:,.0f}/m² — levemente acima da mediana, "
...
def main():
    print("=" * 70)
    print("VIABILIDADE FINANCEIRA — PLACON ARMÍNIO TAVARES")
    print("=" * 70)
    ...
    print("\n[8/8] Montando workbook V2 didático…")
    build_v2_workbook(
        prem, kpis, mc_stats, cenarios, tornado, fluxo, breakdown,
        hist_vpl_png, hist_tir_png, tornado_png,
        gantt_png, curva_s_png, stacked_png, fluxo_png,
