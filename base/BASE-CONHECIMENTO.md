...
> **Última atualização: 14/04/2026 — Fase 18b (calibração condicional via Gemma)**
>
> **📍 Narrativa recente:** Ver `~/orcamentos-openclaw/base/SESSAO-2026-04-14-REVISAO-3-PACOTES.md` para detalhe completo das Fases 14, 16, 17, 18 e 18b (revisão dos 3 pacotes reais + classificação semântica de padrão).
>
> **Calibração (pós-Fase 18b):**
> - `calibration-condicional-padrao.json` — **fonte primária**: medianas por-macrogrupo × classe Gemma (economico/medio/medio-alto/alto/luxo). n_medio-alto=37, n_alto=23.
> - `calibration-indices.json` — global (fallback quando n<3 no bucket condicional)
> - `base-pus-cartesian.json` (1.740 PUs) + **`itens-pus-agregados.json` (4.210 clusters cross-projeto)** — usado pelo PU sanity filter
> - `padroes-classificados-consolidado.json` — 125/126 projetos com label Gemma (fase 18)
> - `base-indices-master-2026-04-13.json` — 322 KB consolidado
>
> **Camada qualitativa Gemma completa:** sub-disciplinas (126), premissas (58), composições (22), observações completas (126), padrões de classificação semântica (125).
>
> **Extração de índices:** `indices-subdisciplina.json` (250 projetos, 5 formatos, 508 índices únicos)
> **Resumo estatístico:** `INDICES-SUBDISCIPLINA-RESUMO.md` (medianas/faixas por subdisciplina)
> PUs detalhados dos projetos 12-25: `archive/PUs-NOVOS-PROJETOS.md`

## Roadmap de Fases (pós-Fase 19 — todas concluídas)

| Fase | Descrição | Status |
|---|---|---|
| 1-13 | Pipeline base V2 (extração → clusters → índices → 18 MGs) | ✅ |
| 14 | Gemma sobre observações completas (126 projetos) | ✅ |
| 15 | base-indices-master consolidado (322 KB) | ✅ |
| 16a-d | Quantitativos físicos BIM/DXF/PDF via bbox/ezdxf/pypdf | ✅ |
| 17 | Memoriais de extração cross-ref a 4.210 PUs | ✅ |
| 18 | Classificação semântica de padrão via Gemma (125/126) | ✅ |
| 18b | Calibração condicional usando labels Gemma (fonte primária do preliminar) | ✅ |
| **19** | **Paramétrico V2 Híbrido** (aba INDICES com override manual + pré-preenchimento BRIEFING via config JSON + 3 novos fatores overridable) | ✅ |

**Doc canônica do Paramétrico V2 Híbrido:** `~/orcamentos-openclaw/base/PARAMETRICO-V2-HIBRIDO.md`

**🔍 Catálogo único consultável** (quando precisar achar rápido qualquer índice da base):
- `~/orcamentos-openclaw/base/INDICES-CATALOGO.xlsx` — 10 abas filtráveis, ~6.500 linhas (PUs cross-projeto V1+V2, calibração global e condicional por padrão Gemma, 29 derivados v2, estruturais, curva ABC, cross insights)
- `~/orcamentos-openclaw/base/INDICES-CATALOGO.md` — guia didático pra engenheiro civil (como ler percentis, 6 cenários de uso)
- Regeneração: `python scripts/gerar_catalogo_indices.py`

**Aba INDICES Híbrida** (6 colunas):
| A | B | C | D | E | F |
|---|---|---|---|---|---|
| Índice | Valor calc (IF BRIEFING) | **Override manual** | Efetivo (IF C="",B,C) | Unidade | Fonte |

Orçamentista sobrescreve qualquer índice na Col C durante reunião com cliente; Col D (efetivo) é consumida por todas as abas de macrogrupo. Pré-preenchimento do BRIEFING via `parametrico-v2-config.json` com 14 chaves (laje, fundacao, padrao_acabamento, fachada, etc).

**3 pacotes entregues nessa onda** (em `~/orcamentos-openclaw/base/pacotes/`):
- arthen-arboris (medio-alto): R$ 41,77M / R$ 3.349/m²
- placon-arminio-tavares (medio): R$ 12,17M / R$ 2.976/m²
- thozen-electra (alto): R$ 157,48M / R$ 4.156/m²
- **Total conjunto: R$ 211,42M**
> **Últimos 3 projetos adicionados (batch 10/mar/2026):** **Pio 4** (Viva4) — **13.541 m²**, **CUB 1,18**, jun/2025 | **Barra 4** (Viva4) — **13.223 m²**, **CUB 1,12**, abr/2024 | **Tramonti** (Amalfi) — **15.603 m²**, **CUB 1,31 (ALTO PADRÃO)**, jun/2025. **DESTAQUES COMUNS:** Cliente Viva4 (Pio4 + Barra4) — portes similares ~13k m². **Climatização inclusa em Sist. Especiais** nos 3 projetos. **Louças e Metais não separados** em Pio4/Barra4 (apenas Tramonti tem separado: R$ 280k, R$ 18/m²). **Infraestrutura alta** em Pio4 (R$ 363/m², 10,51%) e Barra4 (R$ 313/m², 10,08%) — acima faixa típica 4-7%. **Tramonti = MAIOR PORTE DA BASE** (15.603 m², superou Ravello 13.434 m²) com **CUB ratio 1,31** (alto padrão). **Estrutura não separada no Tramonti** — estimativa Infra/Supra 25%/75%. Índices completos: `indices/viva4-pio4-indices.md`, `indices/viva4-barra4-indices.md`, `indices/amalfi-tramonti-indices.md`.

> **Projeto anterior:** **Ravello** (Amalfi) — **13.434 m²** (AC), **CUB 1,06**, Itajaí/SC (Centro), Data-base fev/2024, **42 meses** (R01). **DESTAQUES:** **CUB ratio 1,06 = MUITO eficiente** (entre os mais baixos da base para padrão médio-alto). **Supraestrutura R$ 650/m² (22,20%)** = laje nervurada cubetas com locação até lazer e aquisição para tipos. **Gerenciamento R$ 353/m² (12,07%)** = moderado alto. **Instalações R$ 310/m² (10,60%)** = dentro faixa. **77 unidades** (75 aptos + 2 comerciais), **21 pavimentos** (20 + barrilete), **15 tipos**, **2 elevadores**. **Prazo reduzido:** R00 previa 48 meses, R01 otimizou para 42 meses. **Diversas verbas repassadas pela Amalfi.** Área privativa/AC = 49,51% (6.652 m² AP). Índices completos: `orcamento-parametrico/indices/CTN-ALF-RVL-indices.md`.

> **Mirador de Alicante** (Thozen) — **10.661 m²** (AC), **CUB 1,20**, localização N/D, Data-base dez/2023. **DESTAQUES:** **Infraestrutura R$ 249,27/m² (7,51%) = acima faixa típica** — "blocos super parrudos" (fundação robusta). **Supraestrutura R$ 816,47/m²** — "escoramento, forma super resinado (dobro), aço cortado e dobrado". **Gerenciamento 14,31%** = alto (típico 9-12%). **Acabamentos otimizados:** porcelanato só em box, **esquadrias baratas R$ 208/m²** (base da faixa). **Pintura com epóxi em toda garagem**. **AC/AP ratio 1,83**. Sem dados de prazo/produtividade/quantitativo UR. Climatização não separada. Sem previsão de imprevistos. Tipologias: 125,55 m² e 117,20 m². Índices completos: `orcamento-parametrico/indices/mirador-alicante-indices.md`.

> **Malta Residence** (Nova Empreendimentos) — **7.964 m²** (AC), **CUB 1,28** (direto), Florianópolis/SC, **35 meses**, **2 elevadores sociais**. **DESTAQUES:** **Estrutura de custos separada** — ADM incorporadora R$ 18,26M (terreno + comissões) não entra na calibração. **Rev. Int. Parede R$ 414/m² (10,91%) = 180% acima mediana** — inclui fachada. **Fachada R$ 26/m² (0,68%) = 83% abaixo mediana** — subdivisão diferente do executivo. **Gerenciamento R$ 127/m² (3,34%) = 73% abaixo mediana** — só canteiro físico (projetos/equipe na ADM). **Impermeabilização R$ 126/m² (3,31%) = 78% acima mediana**. **MOE separado** R$ 9,67M (31,98% dos custos diretos). Laje treliçada TR8/TR16 + maciça. Fundação: HC + estaca raiz + blocos. Contenção: estaca raiz 31cm + cortinas. Data-base ago/2025. Índices completos: `orcamento-parametrico/malta-indices.md`.

> **Suíça Home** (NM Empreendimentos) — **1.541 m²** (AC EXPLÍCITO), **CUB 1,21**, Navegantes/SC, **~8-10 UR**, **~7-8 pavimentos**, **18 meses** (prazo mais curto da base). **🔴 MENOR PROJETO DA BASE** — substitui Botânico (3.123 m²) como menor AC por >50%. **DESTAQUES:** **Impermeabilização R$ 128/m² (3,98%) = 2× benchmark** (R$ 40-60). **Gerenciamento R$ 508/m² (15,78%)** = alto por efeito escala. **Fachada R$ 188/m² (5,83%)** = acima benchmark (R$ 90-140). **Fundação SAPATAS** = único ou raríssimo na base (maioria usa hélice/estacas). **ETE própria** (fossa séptica + filtro anaeróbio). **Laje treliçada com EPS**, concreto fck 30 MPa. Esquadrias alumínio Suprema + **PVC preto** (diferenciador). Data-base jan/2023. Sem Climatização/Louças/Imprevistos separados. **Ritmo construção: 86 m²/mês** (AC pequeno + prazo curto). Índices completos: `orcamento-parametrico/suica-home-indices.md`.


---

## PROJETO 54: Pio 4 (Viva4)

| Variável | ID | Valor | Unidade |
|----------|----|-------|---------|
| Cliente | — | Viva4 | — |
| Código CTN | — | CTN-VIV4-PIO4 | — |
| Localização | — | N/D | — |
| Área Construída | AC | **13.540,93** | m² |
| Data-base | — | **Jun/2025** | — |
| CUB na data-base | — | R$ 2.934,53 | R$ |
| **Total** | — | **R$ 46.779.024** | R$ |
| R$/m² | — | **R$ 3.454,64** | R$/m² |
| **CUB ratio** | — | **1,18** | CUB |

### Custos por Macrogrupo — Pio 4 (R$ — Jun/2025)

| Grupo | Valor (R$) | R$/m² | % |
|-------|-----------|-------|---|
| Supraestrutura | 8.968.130 | 662,23 | 19,17% |
| **🔴 Infraestrutura** | **4.918.974** | **363,23** | **10,51%** ⚠️ |
| Instalações | 4.723.509 | 348,83 | 10,10% |
| Gerenciamento | 4.850.508 | 358,22 | 10,37% |
| Sist. Especiais | 3.672.506 | 271,21 | 7,85% |
| Alvenaria | 3.078.523 | 227,33 | 6,58% |
| Esquadrias | 3.129.432 | 231,10 | 6,69% |
| Pisos | 2.623.038 | 193,71 | 5,61% |
| Rev. Int. Parede | 2.265.898 | 167,33 | 4,84% |
| Complementares | 2.117.989 | 156,41 | 4,53% |
| Pintura | 2.040.601 | 150,70 | 4,36% |
| Fachada | 1.567.169 | 115,75 | 3,35% |
| **🔴 Mov. Terra** | **1.013.090** | **74,81** | **2,17%** ⚠️ |
| Teto | 619.561 | 45,76 | 1,32% |
| Imprevistos | 619.633 | 45,76 | 1,32% |
| Impermeabilização | 570.461 | 42,13 | 1,22% |
| Climatização | 0 | 0,00 | 0,00% |
| Louças e Metais | 0 | 0,00 | 0,00% |

**⚠️ DESTAQUES:**
- **Infraestrutura R$ 363/m² (10,51%)** — acima faixa típica (115-344 R$/m², 4-7%). Sugere fundações profundas ou contenções complexas
- **Mov. Terra R$ 75/m²** — topo da faixa (7-85), terreno possivelmente com desnível
- **Sist. Especiais 7,85%** — acima típico (3-7%), inclui climatização
- **Louças e Metais não separados** — dificulta análise detalhada

> Índices completos: `orcamento-parametrico/indices/viva4-pio4-indices.md`

---

## PROJETO 55: Barra 4 (Viva4)

| Variável | ID | Valor | Unidade |
|----------|----|-------|---------|
| Cliente | — | Viva4 | — |
| Código CTN | — | CTN-VIV4-BAR4 | — |
| Localização | — | N/D | — |
| Área Construída | AC | **13.223,24** | m² |
| Data-base | — | **Abr/2024** | — |
| CUB na data-base | — | R$ 2.757,56 | R$ |
| **Total** | — | **R$ 41.008.192** | R$ |
| R$/m² | — | **R$ 3.101,22** | R$/m² |
| **CUB ratio** | — | **1,12** | CUB |

### Custos por Macrogrupo — Barra 4 (R$ — Abr/2024)

| Grupo | Valor (R$) | R$/m² | % |
|-------|-----------|-------|---|
| Supraestrutura | 7.821.695 | 591,51 | 19,07% |
| **🔴 Gerenciamento** | **5.334.855** | **403,38** | **13,01%** ⚠️ |
| Instalações | 4.783.942 | 361,80 | 11,67% |
| Infraestrutura | 4.135.660 | 312,73 | 10,08% |
| Rev. Int. Parede | 2.342.525 | 177,16 | 5,71% |
| Complementares | 2.321.436 | 175,54 | 5,66% |
| Pisos | 2.047.383 | 154,83 | 4,99% |
| Esquadrias | 2.047.908 | 154,83 | 4,99% |
| Pintura | 1.891.218 | 143,02 | 4,61% |
| **🔴 Impermeabilização** | **1.729.338** | **130,78** | **4,22%** ⚠️ |
| Alvenaria | 1.666.623 | 126,05 | 4,06% |
| Sist. Especiais | 1.621.964 | 122,65 | 3,96% |
| Mov. Terra | 994.139 | 75,18 | 2,42% |
| Fachada | 924.892 | 69,94 | 2,26% |
| Teto | 817.422 | 61,82 | 1,99% |
| Imprevistos | 527.192 | 39,87 | 1,29% |
| Climatização | 0 | 0,00 | 0,00% |
| Louças e Metais | 0 | 0,00 | 0,00% |

**⚠️ DESTAQUES:**
- **Gerenciamento R$ 403/m² (13,01%)** — acima típico (7-10%). Sugere ADM incorporadora inclusa ou projeto longo
- **Impermeabilização R$ 131/m²** — muito acima faixa (43-94). Projeto com áreas molhadas extensas ou subsolos profundos
- **Esquadrias 5%** — abaixo típico (7-11%), fachada possivelmente simples
- **Louças e Metais não separados**

> Índices completos: `orcamento-parametrico/indices/viva4-barra4-indices.md`

---

## PROJETO 56: Tramonti (Amalfi)

| Variável | ID | Valor | Unidade |
|----------|----|-------|---------|
| Cliente | — | Amalfi | — |
| Código CTN | — | CTN-ALF-TRM | — |
| Localização | — | N/D | — |
| Área Construída | AC | **15.602,85** | m² 🔴 **MAIOR PORTE DA BASE** |
| Data-base | — | **Jun/2025** | — |
| CUB na data-base | — | R$ 2.934,53 | R$ |
| **Total** | — | **R$ 59.859.420** | R$ |
| R$/m² | — | **R$ 3.836,44** | R$/m² |
| **CUB ratio** | — | **1,31** | CUB 🔴 **ALTO PADRÃO** |

### Custos por Macrogrupo — Tramonti (R$ — Jun/2025)

| Grupo | Valor (R$) | R$/m² | % |
|-------|-----------|-------|---|
| **🔴 Supraestrutura (est.)** | **13.771.629** | **882,65** | **23,01%** ⚠️ |
| Gerenciamento | 7.713.625 | 494,34 | 12,89% |
| Instalações | 6.044.697 | 387,41 | 10,10% |
| Esquadrias | 4.972.981 | 318,67 | 8,31% |
| **🔴 Infraestrutura (est.)** | **4.590.543** | **294,22** | **7,67%** |
| Alvenaria | 3.110.674 | 199,38 | 5,20% |
| Sist. Especiais | 2.681.510 | 171,85 | 4,48% |
| Fachada | 2.575.360 | 165,07 | 4,30% |
| Pisos | 2.503.625 | 160,47 | 4,18% |
| Pintura | 2.472.056 | 158,45 | 4,13% |
| Complementares | 2.465.512 | 158,02 | 4,12% |
| Rev. Int. Parede | 1.879.567 | 120,47 | 3,14% |
| Teto | 1.650.988 | 105,81 | 2,76% |
| **🔴 Mov. Terra** | **1.415.599** | **90,72** | **2,37%** ⚠️ |
| Imprevistos | 928.038 | 59,48 | 1,55% |
| Impermeabilização | 802.415 | 51,43 | 1,34% |
| Louças e Metais | 280.602 | 17,98 | 0,47% ✅ |
| Climatização | 0 | 0,00 | 0,00% |

**⚠️ DESTAQUES:**
- **MAIOR PORTE DA BASE:** 15.603 m² (superou Ravello 13.434 m²)
- **CUB ratio 1,31 = ALTO PADRÃO** (entre os mais altos da base)
- **Estrutura não separada no executivo** — Infra/Supra estimados 25%/75% (proporção típica SC)
- **Supraestrutura (est.) R$ 883/m² (23%)** — topo da faixa, sugere projeto estruturalmente elaborado
- **Mov. Terra R$ 91/m²** — acima faixa (7-85), terreno complexo
- **Louças e Metais separados** — R$ 280k (R$ 18/m²) — diferencial positivo vs Pio4/Barra4
- **Teto R$ 106/m²** — alto, sugere forros elaborados (gesso, sancas)

> Índices completos: `orcamento-parametrico/indices/amalfi-tramonti-indices.md`
...
## PROJETO 54: Mirador de Alicante (Thozen)

| Variável | ID | Valor | Unidade |
|----------|----|-------|---------|
| Incorporador | — | Thozen | — |
| Localização | — | N/D | — |
| Área Construída | AC | **10.661,12** | m² |
| Área Privativa Total | AP | **5.826,00** | m² |
| AC/AP ratio | — | **1,83** | — |
...
| CUB na data-base | — | R$ 2.752,67 | R$ |
| **Total Direto** | — | **R$ 35.380.193,78** | R$ |
| R$/m² Total | — | **R$ 3.318,62** | R$/m² |
| **CUB ratio** | — | **1,20** | CUB |

### Tipologias Identificadas

| Tipologia | Área (m²/un) | Observação |
|---|---|---|
| Tipo 1 | 125,55 | — |
| Tipo 2 | 117,20 | — |

### Custos por Macrogrupo — Mirador de Alicante (R$ — Dez/2023)

| Grupo | Valor (R$) | R$/m² | % |
|-------|-----------|-------|---|
| **🔴 Supraestrutura** | **8.704.543** | **816,47** | **24,60%** |
| Gerenciamento | 5.062.193 | 474,83 | 14,31% |
| Instalações | 2.908.685 | 272,83 | 8,22% |
| **🔴 Infraestrutura** | **2.657.464** | **249,27** | **7,51%** |
| **Rev. Int. Parede** | **2.648.747** | **248,45** | **7,49%** |
| Esquadrias | 2.218.821 | 208,12 | 6,27% |
| Sist. Especiais | 2.076.278 | 194,75 | 5,87% |
| Pintura | 1.891.027 | 177,38 | 5,34% |
| Complementares | 1.634.774 | 153,34 | 4,62% |
| Fachada | 1.586.316 | 148,79 | 4,48% |
| Alvenaria | 1.434.488 | 134,55 | 4,05% |
| Pisos | 1.103.824 | 103,54 | 3,12% |
| Teto | 495.265 | 46,46 | 1,40% |
| Impermeabilização | 441.430 | 41,41 | 1,25% |
| Louças e Metais | 325.262 | 30,51 | 0,92% |
| Mov. Terra | 191.078 | 17,92 | 0,54% |
| Climatização | 0 | 0,00 | 0,00% |
| Imprevistos | 0 | 0,00 | 0,00% |

**Validação:** Soma dos 18 macrogrupos = R$ 35.380.194 ✓

**⚠️ Dados de fonte:**
- Valores principais: aba "ORÇAMENTO_EXECUTIVO" (fonte primária)
- Mov. Terra: aba "ANÁLISE_CUSTOS" (R$ 191.078,05 | R$ 17,92/m²)
- Infraestrutura: ajustada (valor executivo menos mov. terra)
...
| Burn rate | N/D | R$/mês |
| Custo/mês/m² | N/D | R$/m²/mês |

### Destaques Técnicos — Observações da Equipe CTN

**🔴 ESTRUTURA ROBUSTA (custos elevados justificados):**
1. **Infraestrutura R$ 249,27/m² (7,51%)** — acima faixa típica (180-210)
   - Anotação CTN: **"blocos super parrudos"**
   - Fundação robusta/complexa
   
2. **Supraestrutura R$ 816,47/m² (24,60%)** — dentro da faixa, mas elaborada
   - Anotação CTN: **"escoramento, forma super resinado (dobro), aço cortado e dobrado"**
   - Formas com custo dobrado vs padrão
   - Aço cortado e dobrado (não industrializado)

**✅ ACABAMENTOS OTIMIZADOS (economia):**
3. **Esquadrias R$ 208,12/m² (6,27%)** — base da faixa benchmark (244-950)
   - Anotação CTN: **"esquadrias mais baratas"**
   
4. **Pisos R$ 103,54/m²** — otimizado
   - Anotação CTN: **"porcelanato apenas em box - custo do porcelanato"**

**⚠️ PINTURA ESPECIAL:**
5. **Pintura R$ 177,38/m²** — dentro da faixa (84-194)
   - Anotação CTN: **"pintura epóxi em toda garagem"**
   - Adicional vs pintura padrão

**📊 GERENCIAMENTO ELEVADO:**
6. **Gerenciamento R$ 474,83/m² (14,31%)** — alto (típico 9-12%)
   - Pode indicar complexidade de gestão
   - Ou custos administrativos da incorporadora inclusos

**🔍 DADOS AUSENTES:**
- Sem dados de prazo/produtividade
- Sem quantitativo de unidades/vagas/elevadores
- Sem detalhamento de consumos estruturais (concreto/aço/alvenaria)
- Climatização não separada (presumivelmente em Instalações)
- Sem previsão de Imprevistos (0%)

### Adequação para Base Paramétrica

**✅ ADEQUADO para calibração de:**
- Macrogrupos gerais (todos 18 mapeados)
- R$/m² global (R$ 3.318,62 | CUB 1,20)
- Relações estruturais (suprae/infrae)
- Instalações agrupadas
- CUB ratio para padrão médio-alto SC
- Índices de acabamentos otimizados

**⚠️ LIMITAÇÕES:**
- Sem dados de prazo/produtividade
- Sem quantitativo de unidades (impossibilita R$/UR)
- Sem detalhamento de consumos (kg aço/m³, m³ concreto/m²)
- Climatização não separada
- Sem previsão de Imprevistos

**🎯 VALOR para base:**
1. **Observações técnicas da equipe CTN** — RARO ter anotações qualitativas sobre decisões construtivas
2. **Exemplo de otimização de acabamentos** — esquadrias baratas + porcelanato mínimo
3. **Estrutura robusta documentada** — referência para fundações complexas
4. **AC/AP ratio 1,83** — referência para eficiência espacial
5. **CUB ratio 1,20 (dez/2023)** — benchmark para médio-alto SC

**📊 Complementaridade com outros projetos:**
- **Atlantia (H Emp.):** CUB 1,10 vs Mirador 1,20 — espectro de otimização médio-alto SC
- **Malta (Nova):** Gerenciamento 3,34% vs Mirador 14,31% — mostra impacto de estrutura de custos
- **Supraestrutura elaborada** — complementa projetos com estrutura otimizada

---

**Extraído em:** 10/03/2026  
**Código CTN:** CTN-THO-MAT  
**Índices completos:** `orcamento-parametrico/indices/mirador-alicante-indices.md`
